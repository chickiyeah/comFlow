...
export default function AssistantPanel() {
  ...
  const handleOpen = async () => {
    ...
    try {
      ...
      setError(t('ui.assistant.errorRetry'))
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className={`fixed top-0 right-0 h-full w-full sm:w-[480px] z-50
        ...
        <div className="sticky top-0 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 p-5 flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-['Space_Grotesk'] font-bold text-primary dark:text-white">{t('ui.assistant.heading')}</h2>
              <p className="text-label-md text-outline dark:text-slate-400">{t('ui.assistant.tailoredFor', { name: user?.name })}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {data && (
              <button onClick={handleRefresh} className="p-2 rounded-full hover:bg-surface-container dark:hover:bg-slate-800 transition-colors text-outline dark:text-slate-400" title={t('ui.assistant.refresh')}>
            ...
            <div className="space-y-4">
              <div className="flex flex-col items-center py-12 gap-4">
                ...
                <p className="text-sm text-on-surface-variant dark:text-slate-400 text-center">
                  {t('ui.assistant.loading')}<br />
                  <span className="text-label-md">{t('ui.assistant.loadingDetail')}</span>
                </p>
              </div>
              {[1,2,3].map(i => (
            ...
            <>
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <p className="text-white/70 text-sm">{t('ui.assistant.semesterSummary', { current: data.currentSemester, remaining: data.remainingSemesters })}</p>
                    </div>
              ...
              {data.careerStat ? (
                <div className="card p-5">
                  <h3 className="font-['Space_Grotesk'] font-bold text-primary dark:text-white mb-1 flex items-center gap-2">
                    <span className="material-symbols-outlined text-secondary dark:text-secondary-fixed">work</span>{t('ui.assistant.careerTitle')}
                  </h3>
                  <p className="text-label-md text-outline dark:text-slate-400 mb-3">{t('ui.assistant.careerFor', { job: data.desiredJob })}</p>
                  <div className="grid grid-cols-2 gap-2.5 mb-3">
                    <div className="p-3 bg-surface-container-low dark:bg-slate-800 rounded-xl">
                      <p className="text-[11px] text-outline dark:text-slate-400 mb-0.5">{t('ui.assistant.careerSalary')}</p>
                      <p className="font-bold text-sm text-primary dark:text-white">{data.careerStat.expectedSalary ?? '—'}</p>
                    </div>
                    <div className="p-3 bg-surface-container-low dark:bg-slate-800 rounded-xl">
                      <p className="text-[11px] text-outline dark:text-slate-400 mb-0.5">{t('ui.assistant.careerEducation')}</p>
                      <p className="font-bold text-sm text-primary dark:text-white">{data.careerStat.requiredEducation ?? '—'}</p>
                    </div>
                  </div>
                  {data.careerStat.recommendedCerts?.length > 0 && (
                    <>
                      <p className="text-[11px] text-outline dark:text-slate-400 mb-1.5">{t('ui.assistant.careerCerts')}</p>
                      <div className="flex flex-wrap gap-1.5">
                        {data.careerStat.recommendedCerts.map((c, i) => (
                          <span key={i} className="px-2.5 py-1 rounded-full bg-secondary-container dark:bg-secondary-fixed/20 text-on-secondary-container dark:text-secondary-fixed text-xs border border-secondary-fixed/30">{c}</span>
                        ))}
                      </div>
                    </>
                  )}
                ...
                <div className="card p-4 flex items-center gap-3 border border-dashed border-outline-variant dark:border-slate-700">
                  <span className="material-symbols-outlined text-outline dark:text-slate-500">work_outline</span>
                  <p className="text-sm text-on-surface-variant dark:text-slate-400">{t('ui.assistant.careerNoJob')}</p>
                </div>
              )}
              ...
              {data.attendanceWarnings?.length > 0 && (
                <div className="bg-error-container dark:bg-error/20 rounded-2xl p-4 border border-error/30">
                  <h3 className="font-bold text-error flex items-center gap-2 mb-2">
                    <span className="material-symbols-outlined text-[18px]">warning</span>{t('ui.assistant.attendanceWarning')}
                  </h3>
              ...
              {data.certificates?.length > 0 && (
                <div className="card p-5">
                  <h3 className="font-['Space_Grotesk'] font-bold text-primary dark:text-white mb-3 flex items-center gap-2">
                    <span className="material-symbols-outlined text-secondary dark:text-secondary-fixed">workspace_premium</span>{t('ui.assistant.certRecommend')}
                  </h3>
              ...
              {data.studyFocus?.length > 0 && (
                <div className="card p-5">
                  <h3 className="font-['Space_Grotesk'] font-bold text-primary dark:text-white mb-3 flex items-center gap-2">
                    <span className="material-symbols-outlined text-secondary dark:text-secondary-fixed">menu_book</span>{t('ui.assistant.studyFocus')}
                  </h3>
              ...
              {data.portfolioIdeas?.length > 0 && (
                <div className="card p-5">
                  <h3 className="font-['Space_Grotesk'] font-bold text-primary dark:text-white mb-3 flex items-center gap-2">
                    <span className="material-symbols-outlined text-secondary dark:text-secondary-fixed">lightbulb</span>{t('ui.assistant.portfolioIdeas')}
                  </h3>
                  <div className="space-y-2">
              ...
              {data.graduationWarnings?.filter(w => w.shortage > 0).length > 0 && (
                <div className="card p-5">
                  <h3 className="font-['Space_Grotesk'] font-bold text-primary dark:text-white mb-3 flex items-center gap-2">
                    <span className="material-symbols-outlined text-secondary dark:text-secondary-fixed">school</span>{t('ui.assistant.graduationStatus')}
                  </h3>
                  <div className="space-y-2">
                    {data.graduationWarnings.map((w, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-surface-container-low dark:bg-slate-800 rounded-xl">
                        ...
                        <div className="text-right">
                          <p className="font-bold text-sm text-error">{t('ui.assistant.creditsShort', { count: w.shortage })}</p>
